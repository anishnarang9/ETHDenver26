...
import { handleFindRides } from "./handler.js";
...
app.get("/health", async () => ({ ok: true, service: "rider" }));
...
app.post(
  "/api/find-rides",
  { config: { routeId: "api.find-rides" }, preHandler: [enforcer] },
  async (request) => {
    ...
    const result = await handleFindRides({

...
export class OnchainPassportClient implements PassportClient {
  ...
  async getPassport(agent: `0x${string}`) {
    ...
    return {
      owner: data.owner as `0x${string}`, agent: data.agentAddress as `0x${string}`,
      expiresAt: Number(data.expiresAt), perCallCap: BigInt(data.perCallCap.toString()),
      dailyCap: BigInt(data.dailyCap.toString()), rateLimitPerMin: Number(data.rateLimitPerMin),
...
export class OnchainSessionClient implements SessionClient {
  ...
  async getSession(session: `0x${string}`) {
    ...
    if (!data?.owner || data.owner === ZeroAddress) return null;
    return {
      owner: data.owner as `0x${string}`, agent: data.agent as `0x${string}`,
      session: data.sessionAddress as `0x${string}`, expiresAt: Number(data.expiresAt),
      revoked: Boolean(data.revoked),
      scopes: (data.scopes as string[]).map((v) => v.toLowerCase()),
    };
  }

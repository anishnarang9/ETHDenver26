...
/** Determine if a message is "outgoing" (from an agent) vs "incoming" (from human/external) */
function isOutgoing(email: Email): boolean {
  return email.agentId !== "human";
}
...
export function EmailThread({ emails }: { emails: Email[] }) {
  return (
    <div style={{ background: "#111827", borderRadius: "12px", border: "1px solid #1e293b", padding: "14px", maxHeight: 300, overflow: "auto" }}>
      <h3 style={{ margin: "0 0 10px", fontSize: "0.9rem", color: "#94a3b8", display: "flex", alignItems: "center", gap: 6 }}>
      ...
      {emails.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {emails.map((email) => {
            ...
            const DirectionIcon = outgoing ? Send : User;
            const iconColor = agentColors[email.agentId] || "#64748b";
              ...
              <div key={email.id} style={{ padding: "8px 10px", background: "#0f172a", borderRadius: "8px", borderLeft: `3px solid ${agentColors[email.agentId] || "#64748b"}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: "0.72rem", color: "#64748b" }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
                    <DirectionIcon size={11} style={{ color: iconColor, flexShrink: 0 }} />
                    <strong style={{ color: agentColors[email.agentId] || "#94a3b8" }}>{email.from}</strong>
                    {email.to ? ` \u2192 ${email.to}` : ""}
                  </span>
